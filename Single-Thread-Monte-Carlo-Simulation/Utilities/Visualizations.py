...
def plot_deliveries_per_rain_intensity_combined(simulation_runs):
    ...
    plt.title('Deliveries per Rain Level Across Simulation Runs')
    plt.xticks(index)
...
def visualize_combined_simulation_results(simulation_runs):
    ...
    plt.xticks(index, [f'Run {i+1}' for i in range(len(simulation_runs))])
    ...
    for i, simulation in enumerate(simulation_runs):
        ...
        if success_times:
            ...
            sns.kdeplot(success_times, label=f'Run {i+1}', fill=True, alpha=0.2)
    ...
    for i, simulation in enumerate(simulation_runs):
        ...
        if success_times:
            ...
            plt.plot(sorted_times, y_all_attempts, marker='.', linestyle='-', 
                     label=f'Run {i+1} ({len(success_times)}/{len(results)} = {len(success_times)/len(results)*100:.1f}%)')
